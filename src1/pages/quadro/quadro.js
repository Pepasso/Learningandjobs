...
import PopupQuadro from "../../core/components/popup-quadro/popup-quadro";

const QuadroPage = Barba.BaseView.extend({
  namespace: "quadro",
  onEnter: function () {
    ...
    this.nHeader.classList.add('header__main_no-subsection');
  ...
  onEnterCompleted: async function () {
    ...
    this.popup = [...document.querySelectorAll(".popup")].map(popup =>
      new PopupQuadro(`[data-name=${popup.dataset["name"]}]`)
    );
  },
